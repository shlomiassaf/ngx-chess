...
import { FishWrap } from './index';
import { flatMessage } from './util';


function stockfishObservable(sf: Stockfish): Observable<any> {
  const o = new Observable<any>(obs 
    => sf.onmessage = (incoming: string | MessageEvent) => obs.next(incoming));
...
export class FishWrapRxJS extends FishWrap {
  ...
  destroy(): void {
    ...
    if (this.sfSubscription && !this.sfSubscription.isUnsubscribed) {
